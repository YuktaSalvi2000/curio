...
Earlier, parseOutputData was running repeatedly whenever interactionArrays changed, so the code would keep re-scanning all layers and re-processing output multiple times for small interaction updates. 
Also, setInteractionArrays was being triggered during data building loops, which caused multiple state updates and extra renders while the data was still being assembled.
Now the flow is more controlled: interaction values are first fully calculated, and only after that the state is updated once. 
Similarly, parseOutputData is no longer repeatedly triggered for every small change, so it runs far less often and only when necessary. 
This means the system now “computes first, then updates state”, instead of “update state → re-run everything → repeat”.
